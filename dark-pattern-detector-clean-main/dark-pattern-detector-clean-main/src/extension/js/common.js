...
const segments = (element) => {
    ...
    // Simple recursive function to find all text-containing elements
    function findTextElements(node) {
        ...
        if (node.nodeType === Node.ELEMENT_NODE) {
            ...
            // If element has direct text content, include it
            const hasText = Array.from(node.childNodes).some(child => 
                (child.nodeType === Node.TEXT_NODE && child.textContent.trim().length > 0) ||
                (child.nodeType === Node.ELEMENT_NODE && child.children.length === 0 && child.textContent.trim().length > 0)
            );
            
            if (hasText && node.innerText && node.innerText.trim().length > 0) {
                // Check if visible
                try {
    ...
    try {
        ...
        console.error("Error in segments function:", e);
    }
